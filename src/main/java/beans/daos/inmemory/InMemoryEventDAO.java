...
public class InMemoryEventDAO implements EventDAO {
    ...
    public Event getById(long id) {
        return getEventStream().filter(e -> e.getId() == id).findFirst().orElse(null);
    }
    ...
    private Event safeUpdate(Event event) {
        ...
        if (assignedEvents.isEmpty()) {
            ...
            return event.withId(generateUniqId());
        } else
            throw new IllegalStateException(String.format(
                    "Unable to assign auditorium: [%s] for event: [%s] on date: [%s]. Auditorium is assigned for other events: [%s]",
                    event.getAuditorium(), event.getName(), event.getDateTime(), assignedEvents));
    ...
    private long generateUniqId() {
        final long [] id = {0L};
        Stream<Event> evenStream = getEventStream();
        do {
            id[0] = Math.abs(random.nextLong());
        } while (evenStream.filter(e -> e.getId() == id[0]).count() > 0);
        return id[0];
    }
}
